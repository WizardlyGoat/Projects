...
public abstract class Piece
{
	...
	{
		KING("king", 0.0), QUEEN("queen", 9.0), ROOK("rook", 5.0), BISHOP("bishop", 3.0), KNIGHT("knight",
				3.0), PAWN("pawn", 1.0);
	...
	// (for computer) the point worth of the piece based on it's position and what empty squares it can see
	private double positionScore;
	// (for computer) the point worth of the piece based on what pieces it is supporting and threatening
	private double observationScore;
	private int index; // index in pieces array
	private static int nextIndex = 0; // the next available index
	...
	public Piece(int x, int y, Color color, Type type)
	{
		...
		imagePath = color.name + type.name + ".png";
	 ...
	 * 
	 * @param pieceCopy
	 *           the piece that is being copied
	 */
	public Piece(Piece pieceCopy)
	{
		...
		index = pieceCopy.getIndex ( );
	} // end Piece constructor

	/*****************************************************************************************
	 * notCheck makes sure that a desired move doesn't put the player's king in danger
	 * 
	 * @param destinationSquare
	 *           the square this piece is wanting to go to
	 * @param pieces
	 *           the array of all pieces
	 * @return true if the move does not put the king in danger, false otherwise
	 */
	public boolean notCheck( Square destinationSquare, Piece[ ] pieces )
	{
		boolean notCheck = true;
		// copy the board so that the desired move can be tested
		Piece[ ] futurePieces = Game.copyBoard ( pieces );
		// test the move using a copy of the current board
		Game.testMove ( Game.getPieceOnSquare ( position, futurePieces ), destinationSquare, futurePieces );
		// if the enemy king would be captured at the cost of putting the king in check, it's legal
		// ( ( color == Color.WHITE ) ? 20 : 4 ) gets the index for the opposite color king
		if ( futurePieces[ ( color == Color.WHITE ) ? 20 : 4].getPosition ( ).sameSquare ( Square.removed ) )
		{
			// notCheck = true;
		}
		// see if the desired move puts the king in danger
		else if ( Game.inCheck ( futurePieces, color ) )
		{
			notCheck = false;
		}
		return notCheck;
	...
	/*************************************************************************************
	 * canTake determines if the piece on destinationSquare is of the same color or not
	 * 
	 * @param destinationSquare
	 *           the square the piece is trying to get to
	 * @param pieces
	 *           the array of all the pieces
	 * @return true if there is either no piece on destinationSquare or it has an opposite color, false otherwise
	 */
	public boolean canTake( Square destinationSquare, Piece[ ] pieces )
	{
		Piece capturePiece = Game.getPieceOnSquare ( destinationSquare, pieces );
	 ...
	 */
	public boolean notBlocked( Square startSquare, Square destinationSquare, Piece[ ] pieces )
	{
		...
		// ( destination == start ) keeps the row / column same if the row / column doesn't change
		...
		try
		{
			...
			{
				// System.out.println ( "Looking at square (" + x + ", " + y + ")" );
				if ( Game.isPieceOnSquare ( x, y, pieces ) )
				{
					// System.out.println ( "Invalid: The way is blocked" );
					notBlocked = false;
				}
				// look at next square
				x += xDirection;
	 ...
	 */
	public ArrayList<Square> getControlledSquares( Piece[ ] pieces )
	{
	...
	/***********************************************************************************
	 * calculatePositionScore calculates the piece's worth based on its' position and what empty squares it sees
	 * 
	 * @param pieces
	 *           the array of all pieces
	 * @return nothing
	 */
	public void calculatePositionScore( Piece[ ] pieces, ComSettings difficulty, ComputerMove currMove )
	{
		...
		// only award points to pieces on the board
		if ( !getPosition ( ).sameSquare ( Square.removed ) )
		{
			...
			for ( Square s : controlledSquares )
			{
				...
				// subtracting 8 and taking absolute value makes the distance 0 (far from center) through 6 (close to
				// center)
	...
	/************************************************************************************
	 * calculateObservationScore calculates the piece's worth based on what pieces it support or threatens
	 * 
	 * the score based on other pieces is done separately so that it doesn't affect the total point worth
	 * 
	 * @param pieces
	 *           the array of all the pieces
	 * @return nothing
	 */
	public void calculateObservationScore( Piece[ ] pieces, ComSettings difficulty, ComputerMove currMove )
	{
		...
		if ( !getPosition ( ).sameSquare ( Square.removed ) )
		{
			...
			for ( Square s : controlledSquares )
			{
				observedPiece = Game.getPieceOnSquare ( s, pieces );
	 ...
	 */
	abstract double getSquareComfort( Square currPosition, Piece[ ] pieces, ComSettings difficulty );
	 ...
	 */
	abstract double getPieceHesitation( Game game, ComputerMove currMove, ComSettings difficulty );
	 ...
	 */
	abstract boolean hasLegalMove( Square destinationSquare, Piece[ ] pieces );

	/****************************************************************************************
	 * canSupport determines if the piece can reach the supportSquare
	 * 
	 * for determining support and threatening pieces, only squares a piece can theoretically take on
	 * 
	 * @param supportSquare
	 ...
	 */
	abstract boolean canSupport( Square supportSquare, Piece[ ] pieces );
	 ...
	 */
	abstract boolean canSee( Square sightSquare, Piece[ ] pieces );
