 ...
 * 
 *         The Bishop piece moves along the diagonals as long as there are no pieces in the way
 * 
 */
public class Bishop extends Piece
{
	...
	public boolean canSupport( Square supportSquare, Piece[ ] pieces )
	{
		...
		if ( xDiff == yDiff && xDiff != 0 )
		{
			// make sure the way is not blocked
			// make sure the king wouldn't be in danger
			canSupport = ( notBlocked ( getPosition ( ), supportSquare, pieces ) )
					&& ( notCheck ( supportSquare, pieces ) );
		}
		else
		{
			canSupport = false;
