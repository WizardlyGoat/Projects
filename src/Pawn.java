 ...
 * 
 *         - can move 1 space forward (toward the opposite color) any time if there is no piece in the way
 * 
 *         - can move 2 space forward on its first move if there are no pieces in the way
 * 
 *         - can capture a piece of the opposite color that is 1 space forward and on an adjacent column (diagonal)
 * 
 *         - can en passant (capture an adjacent pawn (same row) that has just moved 2 spaces forward)
 * 
 *         - when performing en passant, the pawn still moves 1 diagonal space forward
 * 
 *         - upon reaching the last row, the pawn turns into the player's choice of a Queen, Rook, Bishop, or Knight
 */
...
public class Pawn extends Piece
{
	...
	public boolean hasLegalMove( Square destinationSquare, Piece[ ] pieces )
	{
		...
		boolean capturing = Game.isPieceOnSquare ( destinationSquare, pieces );
	...
	public double getSquareComfort( Square currSquare, Piece[ ] pieces, ComSettings difficulty )
	{
		...
		// the amount of rows between the pawn and the back rank (including the pawn's current row)
	...
	/*******************************************************************************
	 * isPassPawn determines if any enemy pawns can stop this pawn from making it to the back rank
	 * 
	 * @param pieces
	 *           the array of all pieces
	 * @return true if the pawn is a pass pawn, false otherwise
	 */
	public boolean isPassPawn( Piece[ ] pieces, int backRank, int direction )
	{
		...
		for ( int c = x - 1; c <= x + 1; c++ )
		{
			...
			{
				blockingPiece = Game.getPieceOnSquare ( c, r, pieces );
	 ...
	 * @param pieces
	 *           the array of all the pieces
	 * @return enPassantPiece: the pawn that can receive enPassant, null if nonexistent or can't receive enPassant
	 */
	public Piece getEnPassantPiece( Piece[ ] pieces )
	{
		int x = getPosition ( ).getX ( ); // current x position
		int y = getPosition ( ).getY ( ); // current y position
		Piece enPassantPiece = Game.getPieceOnSquare ( x - 1, y, pieces ); // check adjacent square
				...
				&& ( (Pawn) enPassantPiece ).getCanGetEnPassant ( ) ) )
		{
			...
			enPassantPiece = Game.getPieceOnSquare ( x + 1, y, pieces );
			if ( ! ( enPassantPiece != null && enPassantPiece.getType ( ) == Type.PAWN
					&& ( (Pawn) enPassantPiece ).getCanGetEnPassant ( ) ) )
			{
