 ...
 * 
 *         - can castle, in which case can move 2 in the direction of the rook it's is castling with
 * 
 *         - when castling, the rook jumps over the king and lands adjacent to it
 * 
 *         - cannot move into "check" (into a position in which an opponent's piece can take the king)
 * 
...
public class King extends Piece
{
	...
	public boolean hasLegalMove( Square destinationSquare, Piece[ ] pieces )
	{
		...
		else if ( xDiff == 2 && yDiff == 0 && canCastle )
		{
			...
			hasLegalMove = notBlocked && !Game.inCheck ( pieces, getColor ( ) ) && ( notCheck ( inBetweenSquare, pieces ) )
	...
	public double getPieceHesitation( Game game, ComputerMove currMove, ComSettings difficulty )
	{
		...
		Piece[ ] pieces = game.getPastPositions ( ).get ( game.getPastPositions ( ).size ( ) - 1 );
	 ...
	 * 
	 * @param kingSquare
	 *           the square the king is on or will be on
	 * @param pieces
	 *           the array of all the pieces
	 * @return the rook Piece that can castle. returns null if there's no piece or can't castle
	 */
	public Piece getCastleRook( Square kingSquare, Piece[ ] pieces )
	{
		Piece rook = null;
		...
		{
			rook = Game.getPieceOnSquare ( 1, castleRow, pieces );
		}
		else if ( kingSquare.sameSquare ( 7, castleRow ) )
		{
			rook = Game.getPieceOnSquare ( 8, castleRow, pieces );
		}
		// make sure rook is not an empty space; make sure the piece is a rook; make sure the rook can castle
	 ...
	 */
	public boolean canCastleStill( Piece[ ] pieces )
	{
		boolean canCastleStill = false;
		...
		// get first rook and make sure it is on the board
		Piece rook1 = pieces[rook1Index];
		if ( Square.removed.sameSquare ( rook1.getPosition ( ) ) )
		{
			( (Rook) rook1 ).setCanCastle ( false );
		}
		// get second rook and make sure it is on the board
		Piece rook2 = pieces[rook2Index];
		if ( Square.removed.sameSquare ( rook1.getPosition ( ) ) )
		{
			( (Rook) rook1 ).setCanCastle ( true );
		}
		// see if the king and one other rook can still castle
		if ( canCastle && ( ( (Rook) rook1 ).isCanCastle ( ) || ( (Rook) rook2 ).isCanCastle ( ) ) )
		{
			canCastleStill = true;
		}
		return canCastleStill;
	} // end canCastleStill
